...
Pill {
    ...
    // Eww battery: sage green text, no surface; warning and critical
    // keep a faint tint for at-a-glance state.
    readonly property color textColor: root.greenState ? "#afbea2" : root.warningState ? root.thWarn : root.criticalState ? root.thBad : root.thText
    readonly property color pillColor: root.warningState ? root.thWarnBg : root.criticalState ? root.thBadBg : "transparent"
    ...
    readonly property string tooltipText: root.timeRemaining !== 0 ? (root.timeRemaining > 0 ? "Empty in " : "Full in ") + root.timeText : root.status
    ...
    function capacityOf(battery) {
        if (battery.capacity !== undefined)
            return Number(battery.capacity);
        ...
        if (chargeNow !== null && chargeFull !== null && chargeFull !== 0)
            return Math.floor(100 * chargeNow / chargeFull);
        ...
        if (energyNow !== null && energyFull !== null && energyFull !== 0)
            return Math.floor(100 * energyNow / energyFull);
        return 0;
    }
    ...
    function energyOf(battery, key) {
        ...
        if (charge !== null && voltage !== null)
            return charge * voltage / 1000000000000;
        return null;
    }
    ...
    function powerOf(battery) {
        ...
        if (powerNow !== null)
            return Math.abs(powerNow) / 1000000;
        const current = root.number(battery.current_now);
        const voltage = root.number(battery.voltage_now);
        if (current !== null && voltage !== null)
            return Math.abs(current * voltage) / 1000000000000;
        return null;
    }
    ...
    function update(text) {
        ...
        for (const name in devices) {
            ...
            if (device.type === "Battery" && device.scope !== "device")
        ...
        if ((status === "Discharging" || status === "Not charging") && adapter !== null) {
            if (adapter.online === "1" && adapter.status !== "Discharging")
                status = "Plugged";
        }
        if (root.capacity === 100 && status === "Charging")
            status = "Full";
        root.status = status;
        ...
        if (status === "Discharging") {
            if (timeToEmpty > 0)
                remaining = timeToEmpty / 3600;
            else if (haveEnergy && havePower && power !== 0)
                remaining = energyNow / power;
        } else if (status === "Charging") {
            if (timeToFull > 0)
                remaining = -timeToFull / 3600;
            else if (haveEnergy && havePower && power !== 0)
                remaining = -(energyFull - energyNow) / power;
            if (remaining > 0)
                remaining = 0;
        }
    ...
    // Charging shows fa-flash: the previous glyph (U+F5E7) exists in
    // none of the packaged Nerd Fonts.
    Text {
        Layout.alignment: Qt.AlignVCenter
        text: root.alt ? root.icon + " " + root.timeText : root.charging ? root.capacity + "% \uF0E7" : root.status === "Plugged" ? root.capacity + "% " : root.icon + "  " + root.capacity + "%"
    ...
    Process {
        id: statusProcess
        command: ["sh", "-c", "for d in /sys/class/power_supply/*/; do n=${d%/}; n=${n##*/};" + " for f in type scope status capacity energy_now energy_full power_now" + " charge_now charge_full current_now voltage_now time_to_empty_now" + " time_to_full_now online; do if [ -r \"$d$f\" ]; then" + " printf '%s %s %s\\n' \"$n\" \"$f\" \"$(cat \"$d$f\")\"; fi; done; done"]
